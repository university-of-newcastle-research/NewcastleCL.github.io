...
llBehavior <- function(A, b, vMean, vEasy, vHard, t0, behavioralData) {
  ...
  if(is.na(any(behavioralData$like))){
    return(-1e10)
  }
...
ll <- function(x,data,sample=FALSE){
  ...
  out <- llfMRI(beta0 = x["beta0"], beta1 = x["beta1"], beta2 = x["beta2"], noisesd = x["noise"], fMRIdata = as.data.frame(data$fmriData), dm = as.data.frame(data$dm))
...
pars <- c("beta0", "beta1", "beta2", "noise")

priors <- list(
  theta_mu_mean = c(0,0,0,0),
  theta_mu_var = diag(c(1,1,1,1))) 
...
sampled <- run_stage(sampler, stage = "burn",iter = 300, particles = 50, n_cores = 16)
save.image("fmri.RData")


sampled <- run_stage(sampled, stage = "adapt",iter = 1000, particles = 100, n_cores = 16)
save.image("fmri.RData")
